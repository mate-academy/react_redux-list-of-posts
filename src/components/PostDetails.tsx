...
export const PostDetails: React.FC<Props> = ({ post }) => {
  ...
  const commentsDispatch = useAppDispatch();
  const [visible, setVisible] = useState(false);

  function loadComments() {
    setVisible(false);

    commentsDispatch(commentsActions.init(post.id));
  }

  useEffect(loadComments, [commentsDispatch, post.id]);

  const deleteComment = (comment: Comment) => {
    commentsDispatch(commentsActions.deleteItem(comment.id));
  };
