...
import { commentsActions } from '../features/commentsSlice';
...
export const PostDetails: React.FC = () => {
  ...
  function loadComments() {
    ...
    commentsApi.getPostComments(post.id)
      .then((items) => dispatch(commentsActions.setItems(items)))
  ...
  const addComment = async ({ name, email, body }: CommentData) => {
    try {
      ...
      dispatch(commentsActions.addItem(newComment));
    } catch (error) {
  ...
  const deleteComment = async (commentId: number) => {
    dispatch(commentsActions.removeItem(commentId));
