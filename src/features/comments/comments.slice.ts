...
import { loadPostComments, deletePostComment, addPostComment } from './actions';
...
const { actions, reducer } = createSlice({
  ...
  extraReducers: builder => {
    ...
    builder.addMatcher(
      isAnyOf(loadPostComments.fulfilled, addPostComment.fulfilled),
      (state, action) => {
        state.loaded = true;
    ...
    builder.addMatcher(
      isAnyOf(loadPostComments.rejected, addPostComment.rejected),
      state => {
        state.hasError = true;
        state.loaded = true;
        state.submitting = false;
